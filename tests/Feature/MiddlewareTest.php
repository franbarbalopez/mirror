...
it('requires active impersonation', function (): void {
    Route::middleware('mirror.impersonating')->get('/requires-impersonation', fn (): string => 'ok');

    get('/requires-impersonation')->assertForbidden();
});

it('prevents access during impersonation', function (): void {
    Route::middleware('mirror.not-impersonating')->get('/forbidden-while-impersonating', fn (): string => 'ok');
...
it('stops expired impersonation and redirects', function (): void {
    ...
    Route::middleware('mirror.expired')->get('/ttl', fn (): string => 'ok');
...
it('uses the configured expired redirect when no leave url exists', function (): void {
    ...
    Route::middleware('mirror.expired')->get('/ttl-expired-default', fn (): string => 'ok');
...
it('allows requests when impersonation is active but not expired', function (): void {
    ...
    Route::middleware('mirror.expired')->get('/ttl-valid', fn (): string => 'ok');
...
it('allows requests when no impersonation is active', function (): void {
    Route::middleware('mirror.expired')->get('/ttl-inactive', fn (): string => 'ok');

    get('/ttl-inactive')->assertOk()->assertSee('ok');
});

it('allows access when impersonation is required and active', function (): void {
    Route::middleware('mirror.impersonating')->get('/requires-active-impersonation', fn (): string => 'ok');
...
it('allows access when impersonation is prevented and inactive', function (): void {
    Route::middleware('mirror.not-impersonating')->get('/allows-regular-users', fn (): string => 'ok');
